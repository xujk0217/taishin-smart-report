    ...
    <div className="app-shell">
      ...
      <main className="seven-stage-shell">
        ...
        {stage === 'output' && realPlan?.planningOutput && realPlan.calculationSummary && <section className="stage-card">
          <div className="section-title"><div><span className="eyebrow">Strands Presentation Agent · REAL</span><h2>輸出可編輯 PowerPoint</h2><p>Agent 會讀取 Stage2 規劃、計算 artifact、原始 Excel 與範本，生成 python-pptx 程式並輸出 PPTX/XLSX。</p></div><span className="badge real">{realPlan.status}</span></div>
          {ACTIVE_PLANNER_STATUSES.has(realPlan.status) && <div className="analysis-log"><span /><p><strong>{realPlan.progress?.currentStage === 'presentation-render' ? 'Agent 正在生成簡報程式與 PPTX' : '等待 Fargate'}</strong></p><span /><p>本次執行 {formatElapsed(activeRunStartedAt(realPlan), now)} · 目標 30 分鐘內</p></div>}
